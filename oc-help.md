...
## DC
	...
	+ autoscale
	oc get hpa
	oc patch dc gateway-g -p '{"metadata":{"labels":{"mylabel":"true"}}}' -n project-pro
	oc patch dc gateway-g -p '{"spec":{"template":{"metadata":{"labels":{"mylabel":"true"}}}}}' -n project-pro
	oc autoscale dc/gateway-g --min 5 --max 45 --cpu-percent=100 -n project-pro
	oc scale dc gateway-g --replicas=5 -n project-pro
...
# TEMPLATES CATALOG
	...
	# descarga local template sanes-darwin-catalog-pre
	...
	oc process -f darwin-configuration-service.template.yaml --param-file parametros/darwin-configuration-service.params | oc apply -f-
		...
		SPRING_PROFILES_ACTIVE=changeme	
